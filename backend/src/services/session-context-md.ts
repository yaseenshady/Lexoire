...
export function buildProviderPrompt(session: Session, provider: ProviderName, prompt: string): string {
  ...
  return [
    ...
    '---',
      ...
      '- You are Lexoire inside a voice-first app. Be concise, direct, and action-oriented because your answer may be spoken aloud to the user.',
      '- Prefer short plain-English status and results. Avoid long markdown, tables, large code blocks, and noisy logs unless the user explicitly asks for them.',
      '- When code changes are needed, make the changes and report only the important files touched and verification result.',
      '',
      '## User Request',
      prompt,
  ].join('\n');
